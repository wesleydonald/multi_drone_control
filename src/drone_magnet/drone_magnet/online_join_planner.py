...
class OnlineJoinPlanner(Node):
    def __init__(self) -> None:
        ...
        # ATTACH-gated approach (join mode). Without this the planner drives at the payload the
        # instant the drone is armed, so it flies off before the operator ever clicks ATTACH.
        # When gated, the planner holds at a hover over the spawn point until it sees the magnet
        # commanded ON (the RViz ATTACH button), then runs the normal approach/attach sequence.
        self.gate_approach_on_magnet = bool(
            self.declare_parameter('gate_approach_on_magnet', True).value
        )
        self.pre_attach_hover_z = float(self.declare_parameter('pre_attach_hover_z', 1.0).value)
        ...
        # Explicit obstacle-state topics (one MotionCaptureState each, pose.position =
        # centre, twist.linear = velocity). When non-empty these OVERRIDE the prefix
        # pattern AND obstacle_count -- point them at the REAL fleet drones, e.g.
        # ['/drone_0/motion_capture_state', ...], so the approach avoids the actual quads
        # instead of a fake-world stand-in. Empty (default) keeps the /fake_obstacles
        # prefix pattern below, so existing fake-world launches are unchanged.
        _obstacle_topics = [
            str(t) for t in (self.declare_parameter('obstacle_state_topics', ['']).value or [])
            if str(t).strip()
        ]
        if _obstacle_topics:
            self.obstacle_state_topics = _obstacle_topics
            self.obstacle_count = len(_obstacle_topics)
        else:
            self.obstacle_state_topics = [
        ...
        # In join mode the approach is held until ATTACH (magnet ON) is clicked; latched True by
        # magnet_command_in_callback. Non-join modes are never gated.
        self.approach_armed = not (self.gate_approach_on_magnet and self.mission_mode == 'join')
        self.pre_attach_hold_xy: Optional[np.ndarray] = None
        ...
        # Watch the operator's ATTACH click so the join approach only starts once the magnet is
        # armed. The planner never sends ON in join mode, so any ON here is an external ATTACH.
        if self.gate_approach_on_magnet and self.mission_mode == 'join':
            self.create_subscription(String, self.magnet_command_topic, self.magnet_command_in_callback, 10)
        ...
        if self.enable_obstacle_diagnostics:
            for obstacle_index, topic in enumerate(self.obstacle_state_topics):
                self.create_subscription(
                    MotionCaptureState,
                    topic,
    ...
    def magnet_command_in_callback(self, msg: String) -> None:
        """Latch the join approach on the operator's ATTACH click (magnet ON)."""
        if str(msg.data).strip().upper() in ('ON', '1', 'TRUE', 'ENABLE', 'ENABLED'):
            if not self.approach_armed:
                self.get_logger().info('ATTACH received (magnet ON): starting join approach')
            self.approach_armed = True

    def publish_magnet_command(self, command: str) -> None:
        command = str(command).strip().upper()
        # In join mode the operator's ATTACH click owns the magnet. The approach/descent states
        # here call publish_magnet_command('OFF'), which would turn the magnet back OFF right
        # after ATTACH and prevent the weld (manager only welds while magnet_on). Suppress all
        # planner-driven magnet control in join mode and let the ATTACH command stand.
        if self.mission_mode == 'join':
            return
    ...
    def update_state_machine(self) -> None:
        # Held before ATTACH (join mode): stay in the initial approach state without advancing.
        if not self.approach_armed:
            self.condition_start_time = None
            return
    ...
    def build_pre_attach_hold_reference(self, p0: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Join-mode pre-ATTACH hold: station-keep at a hover over the spawn XY (no approach).

        The XY is latched from the first sample so the drone rises straight up over its spawn
        point and stays there until ATTACH; z is a fixed hover so it clears the ground/payload.
        """
        if self.pre_attach_hold_xy is None:
            self.pre_attach_hold_xy = p0[:2].copy()
        hold = np.array(
            [self.pre_attach_hold_xy[0], self.pre_attach_hold_xy[1], self.pre_attach_hover_z],
            dtype=float,
        )
        positions = np.repeat(hold.reshape(1, 3), self.horizon_samples, axis=0)
        velocities = np.zeros_like(positions)
    ...
    def build_reference_arrays(
        ...
        # Before ATTACH (join mode), hold at a hover over the spawn point instead of driving at
        # the payload -- the drone takes off and station-keeps until the operator clicks ATTACH.
        if not self.approach_armed:
            return self.build_pre_attach_hold_reference(p0)
