"""
mpc_three_soft_planner_launch.py
--------------------------------
PLANNER-DRIVEN stack for three_soft.sdf: the centralized cable-suspended load
planner (controller_load_mpc) generates each drone's reference trajectory, and
the per-drone MPC tracks it (reference_source:=planner).

vs mpc_three_soft_launch.py (which uses each drone's internal circle), this:
  - sets reference_source:=planner on every controller, and
  - adds the `planner` node (controller_load_mpc).
...
Notes:
  * planner codegen is isolated (c_generated_code_load_planner/), so it compiles
    alongside the controllers' quad_dynamics without clashing.
...
def generate_launch_description():
    ...
    for i, drone_name in enumerate(DRONE_NAMES):
        ...
        # per-drone MPC tracker — track the planner reference
        nodes.append(Node(
            package='controller_mpc_multi', executable='controller',
    ...
    nodes.append(Node(
        package='controller_mpc_multi', executable='main', name='central_controller',
