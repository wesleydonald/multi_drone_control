...
def takeoff_trajectory(dt):
    ...
    z_traj_takeoff = smooth_trajectory(0.0, 1.0, steps_takeoff)
...
def land_trajectory(dt, init_pose):
    ...
    z_traj_move_back = smooth_trajectory(init_pose[2], 1.0, steps_move_back)  # Move back 1 meter
    z_traj_descend  = smooth_trajectory(1.0, 0.1, steps_descend)  # Move back 1 meter
...
def pickup_object_trajectory(dt):
    take_off_traj, _ = takeoff_trajectory(dt)
    target_x = 1
    target_y = 1
    pickup_z = 0.11
    ...
    x_above = np.linspace(0, target_x , steps_above)
    y_above = np.linspace(0, target_y , steps_above)
    z_above = smooth_trajectory(1.5, 1.5 , steps_above)
    yaw_above = np.linspace(0, target_yaw , steps_above)

    x_down_wait_up = target_x * np.ones(steps_down + steps_wait + steps_up)
    y_down_wait_up = target_y * np.ones(steps_down + steps_wait + steps_up)
    yaw_down_wait_up_drop = target_yaw * np.ones(steps_down + steps_wait + steps_up + steps_drop)
    z_down = smooth_trajectory(1.5, pickup_z, steps_down)
    # z_down = np.linspace(1.5, pickup_z, steps_down)
    z_wait = smooth_trajectory(pickup_z,pickup_z,steps_wait)
    z_up = smooth_trajectory(pickup_z, 1.5, steps_up)

    x_drop = np.linspace(target_x, 0 , steps_drop)
    y_drop = np.linspace(target_y, 0 , steps_drop)
    z_drop = smooth_trajectory(1.5, 1.5, steps_drop)

    x_traj = np.concatenate((x_above, x_down_wait_up, x_drop))
    y_traj = np.concatenate((y_above, y_down_wait_up, y_drop))
    z_traj = np.concatenate((z_above, z_down, z_wait, z_up, z_drop))
    yaw_traj = np.concatenate((yaw_above, yaw_down_wait_up_drop))
...
def smooth_trajectory(z0, zf, steps):
    s = np.linspace(0, 1, steps)
    return z0 + (zf - z0) * (10*s**3 - 15*s**4 + 6*s**5)
