...
p00 = [20,5,h0]; % from 10 0 h0
R   = 2;
p00 = [-R, 0, h0];   % circle centre, so first point = [0,0,h0]

t1   = 20;
step = 0.05;
time2   = (0:step:t1)';
theta_c = linspace(0, 2*pi, length(time2))';
pd2     = [time2, p00 + R*[cos(theta_c), sin(theta_c), zeros(size(theta_c))]];
...
figure(10);
plot3(pd(:,2), pd(:,3), pd(:,4), 'b-', 'LineWidth', 2);
hold on;
plot3(pd(1,2), pd(1,3), pd(1,4), 'go', 'MarkerSize', 20);
plot3(pd(end,2), pd(end,3), pd(end,4), 'rs', 'MarkerSize', 20);
xlabel('x'); ylabel('y'); zlabel('z');
title('Desired trajectory'); grid on; axis equal;
