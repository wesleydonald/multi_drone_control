...
class ELRSInterface(Node):
    def __init__(self):
        ...
        # Magnet override: separate from the main controller ELRSCommand so the
        # electromagnet can be switched without overwriting roll/pitch/throttle/yaw.
        self.magnet_command_topic = self.declare_parameter(
            'magnet_command_topic', '/magnet/ELRSCommand'
        ).value
        self.magnet_channel = int(self.declare_parameter('magnet_channel', 10).value)
        self.magnet_override_value = None
        self.magnet_subscription = self.create_subscription(
            ELRSCommand, self.magnet_command_topic, self.magnet_commands_callback, qos_profile
        )
        self.get_logger().info(
            f'Magnet ELRS override listening on {self.magnet_command_topic}, channel_{self.magnet_channel}'
        )
    ...
    def _packet_index_for_elrs_channel(self, channel_index: int):
        """Map ELRSCommand channel_N fields to the CRSF packet array index."""
        if channel_index < 0:
            return None
        if channel_index <= 3:
            return channel_index
        if channel_index == 4:
            # packet[4] is reserved for arming in this code path.
            return None
        if 5 <= channel_index <= 10:
            return channel_index + 1
        return None

    def _normalised_channel_value(self, msg, channel_index: int) -> float:
        field_name = f'channel_{channel_index}'
        if not hasattr(msg, field_name):
            raise AttributeError(field_name)
        return float(max(-1.0, min(1.0, getattr(msg, field_name))))

    def apply_magnet_override_to_packet(self):
        """Apply the latest magnet aux-channel override while preserving flight channels."""
        if self.magnet_override_value is None:
            return
        packet_index = self._packet_index_for_elrs_channel(self.magnet_channel)
        if packet_index is None:
            self.get_logger().warn(
                f'Invalid magnet_channel={self.magnet_channel}; expected one of 0..3 or 5..10.',
                throttle_duration_sec=2.0,
            )
            return
        for packet_index in range(6, 12): self.packet[packet_index] = self.idle + int(self.magnet_override_value * self.range)

    def magnet_commands_callback(self, msg):
        """Receive magnet-only ELRSCommand from magnet_attachment_manager."""
        try:
            self.magnet_override_value = self._normalised_channel_value(msg, self.magnet_channel)
        except AttributeError:
            self.get_logger().warn(
                f'Magnet command did not contain channel_{self.magnet_channel}; ignoring.',
                throttle_duration_sec=2.0,
            )
            return
        self.apply_magnet_override_to_packet()
        self.get_logger().info(
            f"Magnet ELRS override: channel_{self.magnet_channel}={self.magnet_override_value:+.2f}",
