...
class Controller(Node):
    def __init__(self):
        ...
        self.est_params = np.array([22.0, 0.0, 0.12, 100.0, 100.0, 0.5, 0.531]) # thrust ratio = 45 for sim, 24 for real
    ...
    def control_loop(self):
        ...
        if self.armed and self.current_pose is not None:
            ...
            # )
            self.get_logger().info(
                f"pendulum = {self.pendulum_state}"
            )

            self.get_logger().info(
                f"current pos = {self.current_pose[0:3]}, "
                f"ref pos = {traj_for_mpc[0:3, ref_step]}"
            )
            ...
            self.get_logger().info(
                f"state length = {len(estimated_state_with_control)}, "
                f"u_state before solve = {estimated_state_with_control[13:17]}, "
                f"payload = {estimated_state_with_control[17:21]}"
            )
            ...
            self.get_logger().info(
                f"meas quat = {estimated_state[3:7]}, "
                f"ref quat = {traj_for_mpc[3:7, ref_step]}, "
                f"omega = {estimated_state[10:13]}"
            )
